...
class RobinhoodIntegration(Integration):
    ...
    async def get_tax_documents(self):
        ...
        tax_rows = self.safe_get(data, ["content", "tax_rows", year], "get_tax_documents")
        for row in tax_rows:
            content = self.safe_get(row, ["content"], "get_tax_documents")
            for item in content:
                button_action = self.safe_get(item, ["button_action"], "get_tax_documents")
                if button_action:
                    url = self.safe_get(button_action, ["uri"], "get_tax_documents")
                    filename = self.safe_get(item, ["title", "text", "text"], "get_tax_documents")
